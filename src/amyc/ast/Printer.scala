...
trait Printer {
  ...
  def apply(t: Tree)(implicit printUniqueIDs: Boolean = false): String = {
    ...
    def rec(t: Tree, parens: Boolean = true): Document = t match {
      ...
      case Slice(expr, start, end, step) =>

        val s = if (start.isDefined) apply(start.get) else ""
        val e = if (end.isDefined) apply(end.get) else ""
        val sstep = if(step.isDefined) apply(step.get) else "1"
